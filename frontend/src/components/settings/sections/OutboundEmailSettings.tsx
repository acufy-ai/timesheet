...
import { tenantsAPI, tenantSettingsAPI } from '@/api/endpoints';
import { useMailboxes, useTenantSettings, useUpdateTenantSettings } from '@/hooks';
...
export const OutboundEmailSettings: React.FC = () => {
  ...
  const featuresQuery = useQuery({
    ...
    queryFn: () => tenantsAPI.getMyFeatures().then((r) => r.data),
  });
  ...
  const handleTestSmtp = async () => {
    ...
    setTestDetail('');
    try {
      const res = await tenantSettingsAPI.testSmtp();
      if (res.data.ok) {
        setTestState('ok');
      } else {
        setTestState('fail');
