...
/* ── NavRow: wraps the desktop nav items and renders a morphing
   accent underlay that tracks the active link's bounding rect.
   Purely additive: the link's own `bg-primary/15` active background
   stays in place, this layers a soft glow underneath that animates
   between items. If the rect measurement misfires (mount before
   layout, fonts not yet loaded, etc.) the existing static active
...
const NavRow: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const measure = () => {
      ...
      if (!container) return;
      // The active link paints itself with `bg-primary/15` — find the
      // first child whose computed background-color matches that hue.
      // Simpler: find anchors with aria-current or pathname match.
      const links = container.querySelectorAll<HTMLAnchorElement>('a[href]');
      let active: HTMLElement | null = null;
      links.forEach((a) => {
        try {
          const url = new URL(a.href, window.location.origin);
          if (url.pathname === location.pathname) active = a;
        } catch { /* ignore malformed href */ }
      });
      // If no exact match, take the first prefix match (matches `match` arrays).
      if (!active) {
        links.forEach((a) => {
          if (active) return;
          try {
            const url = new URL(a.href, window.location.origin);
            if (url.pathname !== '/' && location.pathname.startsWith(url.pathname)) active = a;
          } catch { /* ignore */ }
        });
      }
      if (!active) {
  ...
  return (
    <div ref={containerRef} className="relative hidden items-center gap-1 lg:flex">
      <span
        aria-hidden
        style={{
          position: 'absolute',
          ...
          background: 'radial-gradient(ellipse at center, hsl(var(--primary) / 0.18) 0%, transparent 75%)',
          boxShadow: '0 0 18px hsl(var(--primary) / 0.25)',
          transition: 'left 380ms cubic-bezier(0.16, 1, 0.3, 1), width 380ms cubic-bezier(0.16, 1, 0.3, 1), opacity 250ms ease',
        }}
      />
      {children}
    </div>
...
export const TopNavBar: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto flex h-[96px] max-w-[1800px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          {/* ── Left: Logo ── */}
          <Link to="/dashboard" className="shrink-0">
            <AcufyLogo variant="full" height={84} />
          </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            {tenant?.name && (
              <span className="em-serif-plain hidden whitespace-nowrap text-lg font-medium text-foreground md:inline-block">
                {tenant.name}
              </span>
            )}
