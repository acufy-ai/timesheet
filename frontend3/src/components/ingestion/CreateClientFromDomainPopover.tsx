...
// PORT NOTES:
// - Ported verbatim from frontend2/src/components/ui/CreateClientFromDomainPopover.tsx.
//   The ONLY source change required is the cn import path:
//   f2 `import { cn } from '@/lib/utils'` -> f3 `import { cn } from '@/lib/cn'`
//   (confirmed: frontend3/src/lib/cn.ts exports the same clsx+twMerge cn).
// - All behavior preserved exactly: props contract is identical
//   ({open, domain, cascadeCount, existingClients, anchorEl, initialValue,
//   isSubmitting?, onConfirm({name, existing}), onClose}); viewport-aware
//   above/below positioning; resize+capture-phase scroll reposition;
//   open/initialValue-keyed value reset; rAF focus+select; Escape-to-close
//   with stopPropagation; Enter-to-confirm; case-insensitive exact fuzzy
//   match -> "Link to X" vs "Create" button label flip; cascade preamble
//   "All N pending email(s) from <domain> will be assigned"; domain-mapping
//   note for corporate domains; personal-domain (no-mapping) note when
//   domain is empty; "assign N" suffix; "Assigning..." in-flight label.
// - Styling primitives are unchanged because f3 already provides every class
//   and token this component uses (verified against frontend3/src/index.css):
//     * CSS component classes: field-input, action-button, action-button-secondary.
//     * theme-token Tailwind utilities: border-border, bg-card, text-foreground,
//       text-muted-foreground (all backed by f3's --border/--card/--foreground/
//       --muted-foreground HSL tokens).
//     * default-palette utilities (amber-700/300, emerald-400/500/700/300),
//       font-mono, and the shadow-[...] / z-[...] arbitrary values are
//       framework-level Tailwind and theme-independent.
//   No reskin to <Button>/<Input>/<Modal> primitives was needed: the f2 markup
//   already targets the exact f3 CSS-class design system, so reusing the raw
//   <input>/<button> + classes keeps pixel-for-pixel parity. (A <Modal> wrap was
//   deliberately NOT used because this is an anchored, viewport-positioned
//   popover, not a centered modal — wrapping it would break the anchor
//   positioning that is the whole point of the component.)
// - The local ExistingClient interface ({id, name}) is kept inline to preserve
//   the exact self-contained props contract; it is structurally compatible with
//   the f3 IngestionClient/admin client shape (callers pass {id, name}).
// - Nothing was omitted. No f3 hook is used (this is a pure presentational
//   component driven entirely by props), so there are no data-layer assumptions
//   to double-check.
